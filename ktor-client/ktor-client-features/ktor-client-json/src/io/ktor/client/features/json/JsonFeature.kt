    ...
    companion object Feature : HttpClientFeature<Config, JsonFeature> {
        ...
        override fun install(feature: JsonFeature, scope: HttpClient) {
            ...
            scope.requestPipeline.intercept(HttpRequestPipeline.Transform) { request: HttpRequestBuilder ->
                ...
                if (request.contentType() != ContentType.Application.Json) return@intercept
                scope.feature(HttpPlainText)

                val body = request.body
                request.body = OutputStreamBody {
                    val data = feature.serializer.write(body)
                    it.writer(textFeature.defaultCharset).write(data)
                }
            }
